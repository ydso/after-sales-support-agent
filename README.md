...
是否提交本地知识库文档和演示数据，应根据文档版权、隐私和仓库用途决定。

已经上传过真实 API Key 时，仅删除 `.env` 不足以消除泄露风险，还应立即在对应平台重新生成密钥。
...
准备公开开源时，请根据项目用途补充 `LICENSE` 文件；在未添加许可证前，默认不代表允许他人复制、修改或分发项目代码。
